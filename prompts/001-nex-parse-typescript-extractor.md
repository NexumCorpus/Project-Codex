# Codex Prompt 001: Implement nex-parse TypeScript Extractor

## Role
You are implementing the `nex-parse` crate for Nexum Graph — a semantic code coordination system. You are filling in function bodies for a TypeScript semantic extractor that uses tree-sitter 0.26.

## Constraints (MANDATORY — violations will be rejected)
- Do NOT alter any type signatures in `nex-core`. They are authoritative.
...
- `cargo test -p nex-parse` must pass ALL tests in `tests/typescript_extractor.rs`.
- No `unsafe` blocks without documented justification.

## What You Own
You own the implementation details inside these files:
1. `crates/nex-parse/src/typescript.rs` — Replace all `todo!()` calls with working implementations.
2. `crates/nex-parse/src/bridge.rs` — Implement the tree-sitter → rowan bridge (optional for this prompt; the TypeScript extractor can work without rowan if needed for Phase 0, but the bridge module should at least contain the `KindMap` infrastructure).

## Architecture Context
```
nex-core (types) ← nex-parse (this crate) ← nex-graph ← nex-cli
```
`nex-parse` depends on `nex-core` for types. Nothing depends on `nex-parse` yet except tests.

## Type Definitions (from nex-core — READ ONLY)
...
The test file is at `crates/nex-parse/tests/typescript_extractor.rs`. Key tests:
...
## Deliverables
1. `crates/nex-parse/src/typescript.rs` — fully implemented TypeScriptExtractor
2. `crates/nex-parse/src/bridge.rs` — at minimum, the `KindMap` type (full bridge is Phase 0 stretch)
3. All 16 tests passing: `cargo test -p nex-parse`
4. Clean clippy: `cargo clippy -p nex-parse -- -D warnings`
5. Formatted: `cargo fmt -p nex-parse --check`
