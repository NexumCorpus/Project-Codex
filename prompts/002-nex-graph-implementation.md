# Codex Prompt 002: Implement nex-graph CodeGraph

## Role
You are implementing the `nex-graph` crate for Nexum Graph. This crate provides a petgraph-based semantic code graph with construction, query, and diff capabilities. It is the core data structure that `nex diff` operates on.
...
- `cargo clippy -p nex-graph -- -D warnings` must pass.
- `cargo fmt -p nex-graph --check` must pass.
- `cargo test -p nex-graph` must pass ALL tests in `tests/code_graph.rs`.
- No `unsafe` blocks without documented justification.

## What You Own
You own the implementation of all `todo!()` function bodies in:
- `crates/nex-graph/src/lib.rs`

You may add private helper functions or internal modules if needed, but the public API must not change.

## Architecture Context
```
nex-core (types) ← nex-graph (this crate) ← nex-coord, nex-cli
```
`nex-graph` depends only on `nex-core` for types and `petgraph` for the graph structure.

## Type Definitions You Consume (from nex-core — READ ONLY)
...
The test file is at `crates/nex-graph/tests/code_graph.rs`. 18 tests:
...
## Deliverables
1. `crates/nex-graph/src/lib.rs` — all `todo!()` replaced with implementations
2. All 20 tests passing: `cargo test -p nex-graph`
3. Clean clippy: `cargo clippy -p nex-graph -- -D warnings`
4. Formatted: `cargo fmt -p nex-graph --check`
